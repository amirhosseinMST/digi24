...
_END;
                ...
                if (isset($_GET["id"])) {
                    $edite = "SELECT * FROM users WHERE id = '{$_GET["id"]}'";
                    $edite_rusult = $db->query($edite);
                    $admin_edite = $edite_rusult->fetch(PDO::FETCH_ASSOC);
                }
                ?>
                <?php
                if (isset($_POST['edite-admin'])) {
                    if ($_POST['password'] != $_POST['pass_repeat']) {
                        header('location: admin-edite.php?mas_er=پسوردها مطابقت ندارند!');
                        exit();
                    }
                    if (strlen($_POST['password']) < 8) {
                        header('location: admin-edite.php?mas_er=طول پسورد کافی نیست.');
                        exit();
                    }
                    if (trim($_POST['name']) != "" && trim($_POST['last_name']) != "" && trim($_POST['email']) != ""
                        ...
                        $sql = "UPDATE `users` SET `first_name` = :name , `last_name` = :last_name , `email` = :email , `phone` = :phone , `pass` = :password , `role` = :role WHERE id = :id";
                        $stmt = $db->prepare($sql);
                        $params = ['name' => $name, 'last_name' => $last_name, 'email' => $email, 'phone' => $phone, 'password' => $password, 'role' => $role, 'id' => $_GET['id']];

                        if ($stmt->execute($params)) {
                            header("Location:admin-edite.php?tex=با موفقيت ثبت شد &id=" . $admin_edite['id']);
                            exit();
                        } else {
                            header("Location:admin-edite.php?tex=متاسفانه ثبت نشد &id=" . $admin_edite['id']);
                            exit();
                        }
                    } else {
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="input-group  mb-3">
                        ...
                        <input name="name" type="text" class="form-control border-0 rounded-0 none-shadow button-color"
                               value="<?php echo $admin_edite['first_name'] ?>" placeholder="" aria-label="Username"
                    ...
                    <div class="input-group  mb-3">
                        ...
                        <input name="last_name" type="text" class="form-control border-0 rounded-0 none-shadow button-color"
                               value="<?php echo $admin_edite['last_name'] ?>" placeholder="" aria-label="Username"
                               aria-describedby="basic-addon1">
                    </div>
                    <div class="input-group  mb-3">
                        ...
                        <input name="email" type="email" class="form-control border-0 rounded-0 none-shadow button-color"
                               value="<?php echo $admin_edite['email'] ?>" placeholder="" aria-label="Username"
                               aria-describedby="basic-addon1">
                    </div>
                    <div class="input-group  mb-3">
                        ...
                        <input id="phone" onkeyup="validat_phone()" name="phone" type="text" class="form-control border-0 rounded-0 none-shadow button-color"
                               value="<?php echo $admin_edite['phone'] ?>" placeholder="" aria-label="Username"
                               aria-describedby="basic-addon1">
                    </div>
                    <select name="role" class=" none-shadow cursor-pointer form-select mt-3 border-0 rounded-0 button-color mb-2"
                            aria-label="Default select example">
                        <option value="post-admin" <?php if($admin_edite['role'] == 'post-admin') echo 'selected'; ?>>پست ادمین</option>
                        <option value="web-admin" <?php if($admin_edite['role'] == 'web-admin') echo 'selected'; ?>>وبلاگ ادمین</option>
                        <option value="support" <?php if($admin_edite['role'] == 'support') echo 'selected'; ?>>پشتیبانی</option>
                        <option value="super-admin" <?php if($admin_edite['role'] == 'super-admin') echo 'selected'; ?>>سوپر ادمین</option>
                    </select>
                    <div class="input-group  mb-3">
                        <span class="input-group-text title-color border-0 rounded-0" id="basic-addon1"> پسورد جدید </span>
                    ...
                    <div class="input-group  mb-3">
                    <span class="input-group-text title-color border-0 rounded-0"
                          id="basic-addon1"> تکرار پسورد جدید </span>
                    ...
                    <button type="submit" name="edite-admin" class="btn title-color mt-3">ويرايش</button>
                </form>
            </div>
        </div>
    </div>
