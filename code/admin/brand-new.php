...
HTML;
            ...
            if (isset($_POST['add-brand'])) {
                if (trim($_POST['title']) != "" && trim($_FILES['img']['name']) != "") {
                    $title = $_POST['title'];
                    $img_name = rand(1,1000).date('dmyhis').$_FILES['img']['name'];
                    $img_tmp = $_FILES['img']['tmp_name'];
                    move_uploaded_file($img_tmp, "../upload/brands/{$img_name}");
                    $sql = "INSERT INTO `brands` (title , img) VALUES (:title , :img) ";
                    $stmt = $db->prepare($sql);
                    $params = ['title' => $title , 'img' => $img_name];
                    if ($stmt->execute($params)) {
                        header("Location:brand-new.php?tex=با موفقيت ثبت شد ");
                        exit();
                    } else {
                        header("Location:brand-new.php?tex=متاسفانه ثبت نشد ");
                        exit();
                    }
                } else {
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <button type="submit" name="add-brand" class="btn title-color">ایجاد</button>
            </form>
        </div>
    </div>
</div>
